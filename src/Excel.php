...
class Excel {
	...
	private $author = "PrimeSoftware";
	private $description = "Automatically generated by Prime Software";
	private $company = '';
	private $title = "PrimeSoftware Report";
	...
	public function loadFromHTML( $html ) {
		$docDoc = new \DOMDocument();
		$docDoc->loadHTML( $html );

		foreach ( $docDoc->getElementsByTagName( 'table' ) as $table_index => $table ) {
			if ( $table->hasAttribute( 'data-excel-sheet' ) ) {
				$this->sheet[ $table_index ] = new ExcelSheet();
				$sheetName = $table->getAttribute( 'data-excel-sheet-title' );
				if ( empty( $sheetName ) ) {
					$sheetName = "Sheet" . $table_index;
				}
				$this->sheet[ $table_index ]->setTitle( preg_replace( "/[^ \\w]+/", " ", substr( $sheetName, 0, 31 ) ) );
				$this->sheet[ $table_index ]->setDescription( $table->getAttribute( 'data-excel-sheet-description' ) );
	...
	private function _process_header( $table_index, \DOMNode $thead ) {
		...
		$this->sheet[ $table_index ]->setHeaderData( $header_rows );
	...
	private function _process_body( $table_index, \DOMNode $tbody ) {
		...
		$this->sheet[ $table_index ]->setData( $xl );
	}
	...
	private function _process_footer( $table_index, $tfoot ) {
		...
		$num_rows = $this->sheet[ $table_index ]->getNumRows();
	...
	public function create( $output_to = null, $report_name = 'Report' ) {
		...
		foreach ( $this->sheet as $thisSheet ) {
			...
			$active_sheet->SetCellValue( "A2", $thisSheet->getTitle() );
			$active_sheet->SetCellValue( "A3", $thisSheet->getDescription() );
			...
			foreach ( $thisSheet->getHeaderData() as $header_row ) {
			...
			$header_range = "A" . $headerRowNum . ":" . $thisSheet->get_end_column_letter() . ( $headerRowNum + $thisSheet->num_header_rows - 1 );
